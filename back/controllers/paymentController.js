...
const home_url = process.env.PORT;
          ...
          Promise.all([
            ...
            .then(async (result) => {
              ...
              res
                .status(200)
                .redirect(
                  `${process.env.PORT}/checkout/success/${transactionDetail.sale.id}`
                );
            })
            .catch((err) => {
              console.log(err);
              res.status(200).redirect(`${process.env.PORT}/cart`);
            });
        }
      }
    );
...
module.exports.checkoutCancel_get = async (req, res) => {
  try {
    ...
    res.redirect(`${process.env.PORT}/cart`);
  } catch (error) {
    console.log(error);
  }
};
